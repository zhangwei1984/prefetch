...
static int fixed_prefetch_aggressiveness = 0;
static int prefetch_gap = 0;
...
void block_counter_init(struct block_counter *bl)
{
	bl->curr_block_lo = 0;
	bl->curr_block_hi = 0;
	...
	bl->consec_blk_cnt = 0;
	bl->reverse_blk_cnt = 0;
	bl->strided_blk_cnt = 0;
	bl->blk_cnt = 0;
}


void block_counter_update(struct block_counter *bl, off_t start, off_t length)
{
	bl->prev_gap = bl->curr_gap;
	bl->prev_block_lo = bl->curr_block_lo;
	bl->prev_block_hi = bl->curr_block_hi;

	bl->curr_block_lo = start;
	bl->curr_block_hi = start + length;

	if (bl->prev_block_hi == bl->curr_block_lo) {
		bl->consec_blk_cnt += length;
		bl->curr_consec_block_hi = bl->curr_block_hi;
	}

	if (bl->prev_block_lo == bl->curr_block_hi) {
		bl->reverse_blk_cnt += length;
	}

	if (bl->prev_block_hi <= bl->curr_block_lo) {
		bl->curr_gap = bl->curr_block_lo - bl->prev_block_hi;
		bl->curr_gap_dir = 1;
		bl->curr_gap_req_len = length;
	}
	
	if (bl->prev_block_lo >= bl->curr_block_hi) {
		bl->curr_gap = bl->curr_block_hi - bl->prev_block_lo;
		bl->curr_gap_dir = -1;
		bl->curr_gap_req_len = length;
	}

	if (bl->curr_gap == bl->prev_gap) {
		/* Should we check the prev req len? */
		bl->strided_blk_cnt += length;
	}
...
struct loop_file_info {
	...
	ino_t ino;
	struct stat st_buf;
	off_t prev_prefetch_end;
	struct block_counter bl;
};
...
static unsigned long long sync_lookup_found;
static unsigned long long sync_lookup_notfound;
static unsigned long long fetch_lookup_found;
static unsigned long long fetch_lookup_notfound;
...

{
	...
	fprintf(fp,
		"sync_lookup_found = %llu sync_lookup_notfound = %llu fetch_lookup_found = %llu fetch_lookup_notfound = %llu\n",
		sync_lookup_found,
		sync_lookup_notfound,
		fetch_lookup_found,
		fetch_lookup_notfound
		);
	fflush(fp);
}
...
static int xmp_open(const char *path, struct fuse_file_info *fi)
{
	...
	if (fd == -1) {
		...
		if (fstat(fd, &sbuf)) {
			...
			info->prev_prefetch_end = 0;
			block_counter_init(&info->bl);
		}
	}
		    ...
		    struct fuse_file_info *fi)
{
	...
	double consec_pct = 0.0;
	block_counter_update(&info->bl, offset, size);

	if (info->bl.blk_cnt > 0) {
		consec_pct = (double) info->bl.consec_blk_cnt / info->bl.blk_cnt;
	}

	int prefetch_aggressiveness = fixed_prefetch_aggressiveness;

	read_bytes_requested += size;

	if (consec_pct < 0.85 && read_bytes_requested > 1048576 * 4) {
		prefetch_aggressiveness = fixed_prefetch_aggressiveness * 20;
	}

	if (fp_sync) {

#if DEBUG
		fprintf(fp_sync, "cnt %d read %s fd %d inode %ju size %zu off %jd consec: %lf\n", cnt, path, fd, (intmax_t) ino, size, (intmax_t) offset, consec_pct);
		fflush(fp_sync);
#endif
...
#if 1
	off_t i;
	off_t prefetch_block_size = req.size;
	off_t current_end_offset = req.offset + req.size;
	off_t prefetch_start_offset = current_end_offset;

	if (prefetch_start_offset < info->prev_prefetch_end) {
		prefetch_start_offset = info->prev_prefetch_end;
	}

	off_t prefetch_stop_offset = current_end_offset + prefetch_aggressiveness * prefetch_block_size;

	if (prefetch_stop_offset > info->st_buf.st_size) {
		prefetch_stop_offset = info->st_buf.st_size;
	}

	fprintf(fp_sync,
		"Prefetch from offset %ld to offset %ld bytes %ld \n",
		prefetch_start_offset,
		prefetch_stop_offset,
		prefetch_stop_offset - prefetch_start_offset
		);
	fflush(fp_sync);

	int enqueued = 0;

	for (i=prefetch_start_offset; i<prefetch_stop_offset; i+= prefetch_block_size) {

		prefetch_req.offset = i;
		prefetch_req.size = prefetch_block_size;
		...
		info->prev_prefetch_end = i + prefetch_block_size;
...
#elif 0
	for (i=prefetch_gap; i<prefetch_gap + prefetch_aggressiveness; i++) {
		prefetch_req.offset = req.offset + (i + 1) * req.size;
		prefetch_req.size = req.size;
		prefetch_req.sem = NULL;
		prefetch_req.refcnt = 0;
		prefetch_req.p_response = NULL;
		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	if (fp_sync) {
#if DEBUG
		fprintf(fp_sync, "cnt %d write %s fd %d inode %ju size %zu off %jd\n", cnt, path,
			fd, (uintmax_t) ino, size, (intmax_t) offset);
		fflush(fp_sync);
#endif
		cnt++;
	}
...
void *prefetch_thread(void *arg)
{
	...
	FILE *fptime = fopen("fuse_time_loop.log", "w");
	...
	for (;;) {
		...
		if (req.exit_flag) {
			fprintf(fp, "Exit flag set - breaking.\n");
			break;
		}
...
#if DEBUG
		fprintf(fp, "Request fd %d inode %ju ref %d size %zu offset %jd pref %d\n", req.fd, (uintmax_t) req.ino, req.refcnt, req.size, (intmax_t) req.offset, is_prefetch);
		fflush(fp);
#endif
		...
		//uint64_t t_mortar_start, t_mortar_end, t_disk_start, t_disk_end;
		unsigned long long  t_mortar_start, t_mortar_end, t_disk_start, t_disk_end;
		t_mortar_start = gettime_ns();
		get_rc = cache->cache_lookup(cache,
					     &req,
					     &lookup_ent);
		...
		if (get_rc == 0) {
...
#if DEBUG
			fprintf(fp, "Read request inode %ju size %zu offset %jd already cached %zd bytes data at %p.\n",
				(uintmax_t) lookup_ent.key.ino,
				lookup_ent.key.size,
				(intmax_t) lookup_ent.key.offset,
				lookup_ent.rc, lookup_ent.data);
			fflush(fp);
#endif
			t_mortar_end =gettime_ns();
			
			fprintf(fptime, " cache hit fd %d size %zu offset %jd time_mortar %llu\n", 
				req.fd,
				req.size,
				(intmax_t) req.offset,
			t_mortar_end - t_mortar_start);
			...
			if (res == 0) {
				...
				t_disk_start = gettime_ns();
				res = pread(req.fd, aligned_buf, req.size, req.offset);

				if (res < 0) {

					fprintf(fp, "pread fd %d size %zu offset %jd failed to read: %s\n",
						req.fd,
						req.size,
						(intmax_t) req.offset,
						strerror(errno));
				...
				t_disk_end = gettime_ns();
					
				fprintf(fptime, "cache_miss fd %d size %zu offset %jd time_disk %llu\n", 
					req.fd,
					req.size,
					(intmax_t) req.offset,
					t_disk_end - t_disk_start);
...
#if DEBUG				
				fprintf(fp, "Copied %zd bytes data to p = %p\n", ent->rc, aligned_buf);
				fflush(fp);
#endif
			}
...
static void xmp_destroy(void* private_data)
{
	...
	while (circ_enq(&req_queue, &req) < 0) {
		sleep(1);
	}
...
int main(int argc, char *argv[])
{
	...
	if (getenv("AGGRESSIVENESS")) {
		fixed_prefetch_aggressiveness = strtol(
			getenv("AGGRESSIVENESS"), NULL, 0);

		fprintf(fp_sync,
			"Set aggressiveness = %d\n",
			fixed_prefetch_aggressiveness);
	}
	...
	if (circ_init(&req_queue, 2048, sizeof(struct elm_read_req))) {
